...
{
    /// <summary>
    /// Modbus RTU协议报文生成与解析工具
    /// </summary>
    public static class ModbusRtuMessageHelper
    {
        /// <summary>
        ...
        {
            if (response.Length < 5) // 最小响应长度（包含CRC）
                return null;
            ...
            if (response.Length < 3 + byteCount + 2) // +2 for CRC
                return null;
        ...
        public static ushort[]? ParseReadHoldingRegistersResponse(byte[] response)
        {
            ...
            byte byteCount = response[2];
            if (response.Length < 3 + byteCount + 2 || byteCount % 2 != 0) // +2 for CRC
                return null;
        ...
        {
            exceptionCode = 0;
            if (response.Length < 5 || (response[1] & 0x80) == 0) // +2 for CRC
                return false;
